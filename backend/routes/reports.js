...
router.get('/mental-health', (req, res) => {
    try {
        ...
        let trend = 'stable';
        if (moods.length >= 6) {
            const r = moods.slice(0,3), o = moods.slice(-3);
            const ra = r.reduce((s,m)=>s+m.score,0)/3, oa = o.reduce((s,m)=>s+m.score,0)/3;
            if (ra > oa+1) trend = 'improving';
            else if (ra < oa-1) trend = 'declining';
        }
        
        let dRisk = 'low', aRisk = 'low';
        ...
        res.json({ report: {
            ...
            user: { name: user.fullName, age: user.age, institution: user.institution },
            summary: { totalEntries: stats.total, averageMood: stats.avgMood, trend, depressionRisk: dRisk, anxietyRisk: aRisk, crisisAlerts: alerts.length },
            statistics: stats,
            activeRecommendations: recs,
            crisisHistory: alerts,
            resources: { crisis: 'Call 112 for help. Text HELP to +234-806-210-6493.', message: trend==='declining'?'Mood declining - seek support.':trend==='improving'?'Mood improving - keep going!':'Mood stable.' }
        }});
